...
IAsyncOperation<GattDeviceService> GetService( BLEDeviceData& deviceData, uint16_t aServiceUUID, uint16_t aNotifyUUID) {
    ...
    if (serviceresult.Status() != GattCommunicationStatus::Success) {
        ...
        printf("\nFound %d services", numservices);
        for (i = 0; i < numservices; i++) {
            uuid = serviceresult.Services().GetAt(i).Uuid().Data1;
            printf("\n#%d UUID %x %s", i, uuid, uuid == aServiceUUID ? "<- select" : "");
        ...
        {  // Ensure we have access to the device.
            ...
            if (accessStatus == DeviceAccessStatus::Allowed) {
                ...
                if ((characteristics.Size() != 0)
                    ...
                    printf("\n\nFound %d characteristics", numcharacteristics);
                    bool found = false;
                    ...
                    for (int jj = 0; jj < numcharacteristics; jj++)
                    {
                        ...
                        printf("\n# %d %x %s ", jj, thisUUID, propstring.c_str() );
...
unsigned ListDevices( bool show )
{
    if (show) {
        for (unsigned i = 0; i < deviceList.size(); i++)
        {
            if (deviceList[i].name.size() != 0) {
                string mac;
                wstring macheader = L"BluetoothLE#BluetoothLE";
                size_t wheremac = deviceList[i].id.find(macheader);
                if (wheremac != string::npos)
                    mac = to_string(deviceList[i].id.substr(wheremac + macheader.length()));
                else
                    mac = "NA";

                printf("\nDevice #%d %s", i, to_string(deviceList[i].name).c_str());
                printf(" isConnectable %s", deviceList[i].isConnectable ? "true" : "false");
            }
        }
    }
